...
graph TD
    ...
    C --> D[모드 판별<br/>일반 모드 vs 딥러닝 모드]

    D --> E[프롬프트 구성<br/>시스템 프롬프트 +<br/>컨텍스트 + 사용자 입력]

    E --> F[LLM API 호출<br/>GPT-5-nano 또는<br/>더 고급 모델]

    F --> G[응답 생성<br/>AI 모델이<br/>응답 생성]

    G --> H[응답 후처리<br/>형식화 및<br/>메타데이터 추가]

    H --> I[최종 출력<br/>사용자에게<br/>응답 전송]

    D --> J[딥러닝 모드<br/>15배 긴 응답]
    J --> E

    C --> K[벡터 임베딩<br/>사용자 상태<br/>벡터화]
    K --> L[개인화된<br/>학습 경로<br/>생성 - 미래 확장]
```
